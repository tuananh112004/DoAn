...
module.exports.lookupByBarcode = async (req, res) => {
    try {
        // Nhận cả barcode và code từ query
        const barcodeInput = req.query.barcode || req.query.code;
        if (!barcodeInput || typeof barcodeInput !== "string" || !barcodeInput.trim()) {
            return res.status(400).json({ message: "Thiếu mã vạch" });
        }
        // Loại bỏ khoảng trắng và tìm barcode gần đúng (bắt đầu hoặc kết thúc bằng barcode nhập vào)
        const barcodeRegex = new RegExp(barcodeInput.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
